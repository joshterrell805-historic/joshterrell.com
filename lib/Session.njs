...
var User = lib('User'),
    ...
    debug = require('debug')('jt:lib:Session'),
...
Session.login = function login(code, state, cookieCsrf) {
  return new Promise(function(resolve, reject) {
    try {
      ...
      reject(new CError('INVALID_JSON', 'Invalid state Json'));
    }

    if (!code || !state) {
      reject(new CError('MISSING_FIELD',
    ...
    if (!(state && state.csrf && state.csrf === cookieCsrf)) {
      reject(new CError('INVALID_CSRF', 'Invalid Csrf Token'));
    }
...
Session.restore = function restore(sessionId) {
  ...
  return Q('SELECT user_id FROM sessions WHERE id = ? ' +
      ...
      [sessionId, site.config.sessionDuration_s])
